...
import { getAllAdmins, getUserPage, deleteUser, insertUser, updataUser } from '../../../api/user'
...
class userAccount extends React.Component {
    ...
    handDelete = (recore) => {
        ...
        Modal.confirm({
            ...
            onOk() {
                deleteUser({id:recore.id}).then(res => {
                    if (res.errno === 0) {
                        message.success("删除成功！")
                        _this.fungetUserPage(_this.state.pageData)
                    }
                })
            },
        })
    ...
    handAddOrEditor = (data={}) => {
        ...
        if (data.id) {
            Modal.confirm({
                ...
                onOk() {
                    if (_this.state.userData.username && _this.state.userData.name) {
                        ...
                        updataUser(_this.state.userData).then(res => {
                            if (res.errno === 0) {
            ...
            Modal.confirm({
                ...
                onOk() {
                    _this.state.userData.createId = cookie.load("userInfo").id
                    if (_this.state.userData.username && _this.state.userData.name) {
                        insertUser(_this.state.userData).then(res => {
                            if (res.errno === 0) {
